...
@app.post("/admin/send-draft/{draft_id}")
async def admin_send_draft(draft_id: uuid.UUID) -> dict:
    """Temporary admin endpoint to approve and send a draft."""
    from app.services.heyreach_client import get_heyreach_client
    ...
    async with async_session_factory() as session:
        ...
        if not conversation.linkedin_account_id:
            raise HTTPException(status_code=400, detail="Missing LinkedIn account ID")
        ...
        return {
            ...
            "message_preview": draft.ai_draft[:100],
        }


@app.get("/admin/draft/{draft_id}")
async def admin_get_draft(draft_id: uuid.UUID) -> dict:
    """Temporary admin endpoint to view a draft."""
    from sqlalchemy.orm import selectinload
    ...
    async with async_session_factory() as session:
        ...
        return {
            ...
            "lead_name": draft.conversation.lead_name,
        }
