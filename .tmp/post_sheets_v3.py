"""Post gift leads with Send/Edit buttons to Slack (with conversation_id fallback)."""
...
def get_ids(name):
    """Get prospect_id and conversation_id from DB."""
    conn = psycopg2.connect(DB_URL)
    cur = conn.cursor()

    # Find conversation
    search_name = name.split(")")[0]  # handle trailing )
    cur.execute(
        "SELECT id, linkedin_profile_url FROM conversations WHERE lead_name ILIKE %s ORDER BY updated_at DESC LIMIT 1",
        (f"%{search_name}%",)
    )
    row = cur.fetchone()
    if not row:
    ...
    # Try to find prospect by URL
    cur.execute(
        "SELECT id FROM prospects WHERE linkedin_url = %s LIMIT 1",
        (linkedin_url,)
    )
    row = cur.fetchone()
    prospect_id = str(row[0]) if row else None

    # Fallback: find prospect by name
    if not prospect_id:
        cur.execute(
            "SELECT id FROM prospects WHERE full_name ILIKE %s LIMIT 1",
            (f"%{search_name}%",)
        )
        row = cur.fetchone()
        prospect_id = str(row[0]) if row else None
...
async def main():
    ...
    for p in PROSPECTS:
        print(f"\nPosting: {p['name']}...")
        prospect_id, conversation_id = get_ids(p["name"])
        print(f"  prospect_id={prospect_id or 'NONE'}, conversation_id={conversation_id or 'NONE'}")

        if not prospect_id and not conversation_id:
            print(f"  SKIP: No prospect or conversation found for {p['name']}")
            continue
